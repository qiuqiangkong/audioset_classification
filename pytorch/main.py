...
class Attention(nn.Module):
    def __init__(self, n_in, n_out, att_activation, cla_activation):
        ...
        self.att = nn.Conv2d(in_channels=n_in, out_channels=n_out, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=True)
...
class EmbeddingLayers(nn.Module):
    ...
    def __init__(self, freq_bins, hidden_units, drop_rate):
        ...
        self.conv1 = nn.Conv2d(in_channels=freq_bins, out_channels=hidden_units, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False)
        self.conv2 = nn.Conv2d(in_channels=hidden_units, out_channels=hidden_units, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False)
        self.conv3 = nn.Conv2d(in_channels=hidden_units, out_channels=hidden_units, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0), bias=False)
    ...
    def forward(self, input, return_layers=False):
        ...
        x = x[:, :, :, None].contiguous()   # (samples_num, freq_bins, time_steps, 1)
        
        a0 = self.bn0(x)
        a1 = F.dropout(F.relu(self.bn1(self.conv1(a0))), p=drop_rate, training=self.training)
        a2 = F.dropout(F.relu(self.bn2(self.conv2(a1))), p=drop_rate, training=self.training)
        emb = F.dropout(F.relu(self.bn3(self.conv3(a2))), p=drop_rate, training=self.training)
...
class DecisionLevelSingleAttention(nn.Module):
    ...
    def __init__(self, freq_bins, classes_num, hidden_units, drop_rate):
        ...
        self.emb = EmbeddingLayers(freq_bins, hidden_units, drop_rate)
        self.attention = Attention(hidden_units, classes_num, att_activation='sigmoid', cla_activation='sigmoid')
...
def train(args):
    ...
    if model_type == 'decision_level_max_pooling':
        ...
        '''
        model = DecisionLevelMaxPooling(freq_bins, classes_num, hidden_units, drop_rate)
